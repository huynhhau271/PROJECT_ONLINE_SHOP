...
test.beforeEach(async ({ page }) => {
  ...
  test('TC1 - Customers: Nhập mới thành công', async ({ page }) => {
    ...
    const lastname = 'Văn A 1234';
    const phone = '090571237';
    const email = 'nguyenvana1234@gmail.com';
    const address = '38 Yên Bái ĐNang';
    const birthday = '08 Jan 2001';
  ...
  test('TC2 - Customers: Cập nhật thành công', async ({ page }) => {
    ...
    const phone = '090571784';
    const email = 'newemail20231234@gmail.com';
    const address = '38 Yên Bái Đà Nẵng';
    const birthday = '19 Jan 2006';
    ...
    await page.getByLabel('Update Customer').getByLabel('FirstName').fill(firstname);
    await page.getByLabel('Update Customer').getByLabel('FirstName').fill(lastname);
    await page.getByLabel('Update Customer').getByLabel('Phone').fill(phone);
    await page.getByLabel('Update Customer').getByLabel('Email').fill(email);
    await page.getByLabel('Update Customer').getByLabel('Address').fill(address);
    await page.getByLabel('Update Customer').getByLabel('Birthday').fill(birthday);
