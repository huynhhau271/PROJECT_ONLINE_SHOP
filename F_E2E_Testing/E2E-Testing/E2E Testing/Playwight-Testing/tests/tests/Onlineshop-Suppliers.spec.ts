...
test.beforeEach(async ({ page }) => {
    ...
    // Vào đúng đường dẫn
    await page.goto('https://os-admin.aptech.io/online-shop/data-management/suppliers');
  });

  test('TC1 - Suppliers: Nhập mới thành công', async ({ page }) => {
    // Khai báo biến
    const name = 'Name 8888';
    const phone = '09058888';
    const email = 'email8888@gmail.com';
  ...
  test('TC2 - Suppliers: Cập nhật thành công', async ({ page }) => {
    // Khai báo biến
  ...
  test('TC3 - Suppliers: Xóa thành công', async ({ page }) => {
    // Dòng đầu tiên
    const firstRow = await page.locator('.ant-table-row-level-0:nth-child(1)');
